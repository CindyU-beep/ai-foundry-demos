...
def create_journalism_research_agent():
    ...
    print("Using Bing search tool for grounding.")
    ...
    writer_agent = project_client.agents.create_agent(
        ...
        instructions="You are a high-quality journalist agent who excels at writing a first draft of an article as well as revising the article based on feedback from the other agents. Do not just write bullet points on how you would write the article, but actually write it. Don't ask if it's necessary to revise the article when asked, just write the article with the reccomendations.",
    ...
    verifier_agent = project_client.agents.create_agent(
        model=os.environ["MODEL_DEPLOYMENT_NAME"],
        name="verifier_agent",
        instructions="You are responsible for ensuring the article's accuracy. You should use the Bing tool to search the internet to verify any relevant facts. Ask the writer for rewrites if you find inaccuracies. Cite your sources and pass back to writer for inclusion in the final article.",
        tools=bing.definitions,
        headers={"x-ms-enable-preview": "true"},
    )
    ...
    orchestrator_agent = project_client.agents.create_agent(
        ...
        instructions="You are leading a journalism team that conducts research to craft high-quality journalist articles. You ensure that the output contains an actual well-written article, not just bullet points on what or how to write the article. If the article isn't to that level yet, ask the writer for a rewrite. If the team has written a strong article with a clear point that meets the requirements, and has been reviewed by the editor, and has been fact-checked and approved by the verifier agent, then respond with a completion message.",
        tools=bing.definitions,
        headers={"x-ms-enable-preview": "true"},
    )
...
def main():
    ...
    with project_client:
        ...
        console.print("\nStep 4: Verifier agent checks the facts using Bing Search...", style="bold magenta")
