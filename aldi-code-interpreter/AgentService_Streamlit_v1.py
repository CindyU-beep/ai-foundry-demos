...
# Set page config
st.set_page_config(
    page_title = "Agent Service - Aldi Demo",
    page_icon = ":ninja:",
    layout = "wide"
)
...
# Main screen
st.title("AI Foundry's Agent Service: Aldi Demo")

if menu == "Code Interpreter":
    st.header("Solving challenging problems with sandboxed Python code")
    default_prompt = "Could you please analyze the quarterly sales performance of Aldi using the following data and produce a bar chart image?\nAldi Sales Performance:\n- Quarter 1: $2.2 million\n- Quarter 2: $2.75 million\n- Quarter 3: $2.3 million\n- Quarter 4: $3.8 million\n- Industry Average: $2.5 million\nWhen quarter values in 2024 fall below the industry average, please highlight them in red, otherwise they should be green."
    prompt = st.text_area("Enter your prompt:", value=default_prompt, height=200)
    if st.button("Run"):
        st.session_state.progress = 0
        result = code_interpreter(prompt)
        st.text_area("Output:", value=str(result), height=70)
    ...
    default_prompt = "Can you provide a summary of Aldi's sustainability initiatives in 2024, including key highlights? I need 3 paragraphs to describe the efforts and achievements."
